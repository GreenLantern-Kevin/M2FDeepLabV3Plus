...
class ModalityElasticFusion(nn.Module):
    """
    【创新点 3 升级版：终极创新模块：跨模态空间-通道联合注意力融合 (CMSCF)】
    解决多模态负迁移问题：利用优质的 RGB 特征作为空间向导，过滤 DEM 噪声。
    """
    def __init__(self, rgb_ch, topo_ch, out_ch):
        ...
        # 2. 深度融合卷积 (使用 3x3 替代 1x1，增加局部感受野，平滑特征)
...
def single_deeplabv3plus_resnet50(num_classes=2, output_stride=8, pretrained_backbone=True):
    ...
    classifier = DeepLabHeadV3Plus(in_channels=2048, low_level_channels=256, num_classes=num_classes, aspp_dilate=aspp_dilate)
