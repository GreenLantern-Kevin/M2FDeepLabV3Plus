...
class M2FValResize(object):
    """
    让验证集尺寸与训练集 Base Size 对齐，消除“尺度错配”导致的漏检！
    """
    ...
    def __call__(self, img, dem, mask, edge):
        ...
        img = F.resize(img, self.size, InterpolationMode.BILINEAR)
        dem = F.resize(dem, self.size, InterpolationMode.BILINEAR)
        # 绝不瞎转 numpy，用 NEAREST 完美保留 0, 1, 255！
