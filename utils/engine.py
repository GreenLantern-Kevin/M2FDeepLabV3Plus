            ...
            if getattr(opts, 'use_tta', False):
                ...
                for i in range(preds.shape[0]):
                    ...
                    for label_idx in range(1, num_labels):
                        # 把写死的 400 改成受终端参数控制（默认设为 0，即不清洗）
                        # 滑坡是巨型地质灾害，小于一定像素的预测斑块 100% 是网络产生的假阳性，直接归零！
                ...
                # 正常标准的验证模式
                probs = torch.nn.functional.softmax(pred_mask.detach(), dim=1)
                preds = (probs[:, 1, :, :] > 0.5).cpu().numpy().astype(int)
            # =========================================================================
