...
def build_optimizer(opts, model):
    ...
    for name, param in model.named_parameters():
        ...
        if strategy == "SINGLE_STANDARD":
            ...
            # 引入一个智能判断：如果你手动设置了极小的学习率 (如 <= 0.001)，说明你在做极限微调
            # 微调时，解开所有骨干网络的封印，全员火力全开！
            if opts.lr <= 0.001:
                scratch_params.append(param)
            else:
                # 正常从头训练时，保护预训练骨干 (0.1x)
...
def validate(opts, model, loader, device, metrics, criterion=None, epoch=None, total_epochs=None):
    ...
    with torch.no_grad():
        ...
        for images, dems, labels, edges, has_dem in pbar:
            ...
            # 2. TTA (Test-Time Augmentation) 模块
            # 通过 opts.use_tta 独立控制。遥感 DEM 场景按需开启。
            if getattr(opts, 'use_tta', False):
                # 水平翻转图像
                ...
                # TTA 开启时，预测更自信，可以考虑使用高于0.5的阈值保 Precision (精确率)，这里没有改
                preds = (probs[:, 1, :, :] > 0.5).cpu().numpy().astype(np.uint8)
            else:
                probs = probs_orig
                # TTA 关闭时，使用 0.5 的基础阈值保 Recall (召回率)
                preds = (probs[:, 1, :, :] > 0.5).cpu().numpy().astype(np.uint8)

            # 3. 连通域清洗模块 (小面积过滤)
            # 由 opts.min_area 独立控制，只要大于 0 就触发
            min_area = getattr(opts, 'min_area', 0)
            if min_area > 0:
                import cv2
                import numpy as np
                # 遍历 Batch 中的每一张预测图
