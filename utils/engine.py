...
def build_optimizer(opts, model):
    """
    【核心黑科技：多模态差分学习率优化器】
    精准分离“已收敛的 RGB 层”与“未收敛的 DEM/融合层”，对症下药！
    """
    pretrained_params = []
    scratch_params = []
    ...
    for name, param in model.named_parameters():
        ...
        # 如果是多模态双流模型
            ...
            # 【只有 RGB 主干是预训练的神装，需要保护】
            if 'rgb_backbone' in name:
                pretrained_params.append(param)
            # 【DEM 主干、融合模块、边缘分支、解码器，全都需要大步快跑】
            else:
                scratch_params.append(param)

    # 打印一下参数分配情况，让你安心
    print(f"[Optimizer] 保护预训练层参数: {len(pretrained_params)} 个 (LR: {opts.lr * 0.1})")
    print(f"[Optimizer] 全力冲刺新层参数: {len(scratch_params)} 个 (LR: {opts.lr})")

    # 构建优化器
    optimizer = torch.optim.SGD([
        {'params': pretrained_params, 'lr': opts.lr * 0.1}, # RGB 主干降速 10 倍
        {'params': scratch_params, 'lr': opts.lr}           # DEM/融合模块保持正常 LR
...
def validate(opts, model, loader, device, metrics, criterion=None, epoch=None, total_epochs=None):
    ...
    with torch.no_grad():
        ...
        for images, dems, labels, edges, has_dem in pbar:
            ...
            preds = pred_mask.detach().max(dim=1)[1].cpu().numpy()
