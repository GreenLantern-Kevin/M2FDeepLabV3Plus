...
class StreamSegMetrics(_StreamMetrics):
    ...
    def get_results(self):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            tn = hist.sum() - (tp + fp + fn)  # 【新增】计算 True Negatives
            ...
            # 【新增】MCC (Matthews Correlation Coefficient) 计算
            # 转为 float64 防止像素数量级过大相乘导致溢出
            tp_f, fp_f, fn_f, tn_f = tp.astype(np.float64), fp.astype(np.float64), fn.astype(np.float64), tn.astype(np.float64)
        ...
        f1 = np.nan_to_num(f1)
        mcc = np.nan_to_num(mcc)  # 【新增】处理 mcc 的 nan
        ...
        cls_mcc = dict(zip(range(self.n_classes), mcc))  # 【新增】封装 MCC
