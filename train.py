        ...
        if opts.continue_training:
            ...
            for old_k, v in old_state_dict.items():
                ...
                if old_k.startswith("backbone."):
                    # 【核心修复】：兼容 Xception (.net.) 和 ResNet 两种单流保存格式
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str,default="weight/best_m2f_deeplabv3plus_xception16_os8_num2.pth", help="用于加载预训练/中断权重，utils/engine.py中build_optimizer 函数已设置动态分配 LR 策略，对权重文件命名有讲究，最好直接加载这个代码训练的权重")
    ...
    parser.add_argument("--beta", type=float, default=0.1, help="幻觉一致性损失 (Consistency Loss) 的权重")
