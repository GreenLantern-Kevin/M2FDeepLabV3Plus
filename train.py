...
def main(opts):
    ...
    for epoch in range(start_epoch, opts.epochs):
        ...
        for images, dems, labels, edges, has_dem in pbar:
            ...
            elif opts.loss_type == "focal_dice" or opts.loss_type == "focal_dice_boundary":
                # 即使是 focal_dice_boundary，loss_seg 也只算主体的 focal + dice
                loss_focal = criterion_focal(pred_mask, labels)
                loss_dice = criterion_dice(pred_mask, labels)
                loss_seg = 0.7 * loss_focal + 0.3 * loss_dice
            # 【核心加入】：专为 M2F 打造的稳定版复合 Loss！
            elif opts.loss_type == "ce_dice_boundary": 
                # 用极其稳定的 CE 替代暴躁的 Focal
                loss_ce = criterion_ce(pred_mask, labels)
                # 针对滑坡小目标的 Dice
                loss_dice = criterion_dice(pred_mask, labels)
                # 你的核心创新点：辅分支边界 Loss
                loss_boundary = criterion_boundary(pred_edge, edges)
                
                # 0.6的主干 + 0.3的形状 + 0.1的边界引导
                loss_seg = 0.6 * loss_ce + 0.3 * loss_dice + 0.1 * loss_boundary
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="m2f_deeplabv3plus_resnet50", choices=available_models,
                        help="模型名称，对应 network.modeling 里的工厂函数")
    parser.add_argument("--ckpt", type=str,default="weight/best_m2f_deeplabv3plus_resnet50_os8_num2.pth", help="用于加载预训练/中断权重，utils/engine.py中build_optimizer 函数已设置动态分配 LR 策略，对权重文件命名有讲究，最好直接加载这个代码训练的权重")
    parser.add_argument("--continue_training", action="store_true", default=False, help="是否从 ckpt 继续训练（恢复 optimizer/scheduler/cur_itrs）")
    parser.add_argument("--loss_type", type=str, default="ce_dice_boundary", choices=["cross_entropy", "focal_loss", "dice_loss", "focal_dice", "focal_dice_boundary", "ce_dice_boundary"], help="损失函数类型")
    ...
    parser.add_argument("--lr", type=float, default=0.001, help="基础学习率（backbone 用 0.1*lr，classifier 用 lr）")
    ...
    parser.add_argument("--beta", type=float, default=0.0, help="幻觉一致性损失 (Consistency Loss) 的权重")
