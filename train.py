...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str,default="", help="用于加载预训练/中断权重")
    parser.add_argument("--continue_training", action="store_true", default=False, help="是否从 ckpt 继续训练（恢复 optimizer/scheduler/cur_itrs）")
    parser.add_argument("--loss_type", type=str, default="focal_loss", choices=["cross_entropy", "focal_loss", "dice_loss", "focal_dice", "focal_dice_boundary"], help="损失函数类型")
    ...
    # ---------------- M-E DeepLab 特有超参数 ----------------
    parser.add_argument("--alpha", type=float, default=0, help="边缘损失 (Edge Loss) 的权重")
    parser.add_argument("--beta", type=float, default=0, help="幻觉一致性损失 (Consistency Loss) 的权重")
