...
def main(opts):
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        if opts.continue_training:
            ...
            # 【核心黑客科技】：如果我们传入的是 VOC 预训练单流权重，进行双流移植！
            print(f"[!] 检测到外部预训练权重，正在进行双流骨干 (RGB & DEM) 权重移植...")
            new_state_dict = model.state_dict()
            transplant_count = 0
            
            for old_k, v in old_state_dict.items():
                # 我们只提取 VOC 权重中的 backbone 部分
                if old_k.startswith("backbone."):
                    base_k = old_k[len("backbone."):] # 例如: conv1.weight
                    ...
                    # 2. 注入到 dem_backbone (赋予其初始的边缘检测能力)
            ...
            # 使用 strict=False 加载，允许 classifier 和 M2F 融合模块随机初始化
            model.load_state_dict(new_state_dict, strict=False)
            print(f"[+] 骨干网络权重移植完成！共成功映射 {transplant_count} 个参数张量。")
            print("[+] Decoder 和 多模态融合模块 将从零开始学习。")
    else:
    ...
    for epoch in range(start_epoch, opts.epochs):
        ...
        print("Validation result: ", format_seg_metrics_line(val_score, use_color=True), "\n")
...
if __name__ == "__main__":
    ...
    # ---------------- 训练超参数（按 epoch） ----------------
    parser.add_argument("--data_root", type=str, default="BijieOnlyLandslide", help="数据集根目录路径")
    ...
    parser.add_argument("--ckpt", type=str,default="weight/best_deeplabv3plus_xception16_os16_num21.pth", help="用于加载预训练/中断权重")
    ...
    parser.add_argument("--epochs", type=int, default=1000, help="训练总轮数（epoch 数），默认 50")
    parser.add_argument("--lr", type=float, default=0.002, help="基础学习率（backbone 用 0.1*lr，classifier 用 lr）")
    ...
    parser.add_argument("--val_batch_size", type=int, default=4, help="验证 batch size（VOC + 不 crop 时会被强制改为 1）")
