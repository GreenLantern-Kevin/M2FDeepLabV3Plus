...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_tta", action="store_true", default=False, help="加入一个参数开关判断是否开启TTA；TTA和连通域清洗已分开这个参数不影响--min_area，TTA 和 连通域清洗 属于推理后处理，只改变模型输出的最终画面，绝对不会参与反向传播，也不会改变网络学习的梯度和权重")
    parser.add_argument("--min_area", type=int, default=100, help="设置连通域清洗像素阈值，把小于这个值的预测斑块判定为网络产生的假阳性，直接归零，已与TTA分开，--use_tta开启与否这个值都起作用，TTA 和 连通域清洗 属于推理后处理，只改变模型输出的最终画面，绝对不会参与反向传播，也不会改变网络学习的梯度和权重")
