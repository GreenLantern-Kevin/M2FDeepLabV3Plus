...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_tta", action="store_true", default=False, help="加入一个参数开关判断是否开启TTA 和连通域清洗")
    parser.add_argument("--min_area", type=int, default=100, help="设置连通域清洗像素阈值，把小于这个值的预测斑块判定为网络产生的假阳性，直接归零，不开起--use_tta时这个值不起作用")
    parser.add_argument("--eval_only", action="store_true", default=False, help="仅加载权重进行验证，绝对不触发任何训练更新")
    
    # 加入这两个新的可调参数
    parser.add_argument("--bce_weight", type=float, default=0.4, help="ce_dice_boundary 中 bce 的权重")
    parser.add_argument("--dice_weight", type=float, default=0.6, help="ce_dice_boundary 中 dice 的权重")
