...
@torch.no_grad()
def infer_one_image(model, model_name, device, img_path: Path, dem_dir: Path):
    """ 推理（兼容单流基线与双流多模态） """
    # 1. 加载并处理 RGB
    pil_img = Image.open(img_path).convert("RGB")
    x_rgb = preprocess_rgb(pil_img).to(device)
    ...
    if "single" in model_name:
        # 单流模型：只传 RGB，直接返回预测 Tensor
        logits = model(x_rgb)
    else:
        # 多模态模型：传 RGB 和 DEM，提取主分支字典的 'seg'
...
def run_on_images(model, model_name, device, input_path: Path, dem_dir: Path, out_dir: Path,
    ...
    for img_path in tqdm(img_list, desc="Infer Images", ncols=100):
        # 【修改点】：把 model_name 传进去
        pred, pil_img = infer_one_image(model, model_name, device, img_path, dem_dir)
...
def main(opts):
    ...
    run_on_images(
        model=model, 
        model_name=opts.model,   # 【新增】：把命令行里的 model 名字传进去
        device=device, 
        input_path=in_path, 
        dem_dir=dem_path, 
        out_dir=exp_dir,
        ...
        save_overlay=opts.save_overlay, 
        alpha=opts.alpha
    )
    print("[Predict] Done.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str,
                        default=r"run/train/exp23/weights/best_single_deeplabv3plus_xception16_os8_num2.pth",
                        help="模型权重路径")
    parser.add_argument("--model", type=str, default="single_deeplabv3plus_xception16", choices=available_models)
